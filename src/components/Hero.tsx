...
export function Hero({ movies, onMovieClick }: HeroProps) {
  ...
  const scaleValue = useTransform(scrollY, [0, 500], [1, 1.1]);
  ...
  return (
    <div ref={containerRef} className="relative w-full h-[80vh] min-h-[600px] lg:h-[95vh] shrink-0 flex flex-col justify-end overflow-hidden font-sans group">
      ...
      >
        {trailerId && !isMobile && (
          <div className="absolute inset-0 w-full h-full overflow-hidden">
            <iframe
              src={`https://www.youtube.com/embed/${trailerId}?autoplay=1&mute=1&controls=0&showinfo=0&rel=0&modestbranding=1&playsinline=1&loop=1&playlist=${trailerId}&vq=hd1080`}
      ...
      {/* Main Content Area */}
      <div className="relative z-20 w-full max-w-[1600px] mx-auto flex flex-col md:flex-row items-center md:items-end pb-16 sm:pb-24 lg:pb-32 px-6 sm:px-10 lg:px-16 gap-8 sm:gap-12 lg:gap-20">
        
        {/* Text and Actions */}
        <div className="flex-1 w-full max-w-3xl flex flex-col relative z-20 pt-32 md:pt-0">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <h1 
                className="mb-4 max-w-4xl text-left leading-[1.05] break-words text-white text-5xl sm:text-7xl lg:text-[5.5rem] drop-shadow-[0_10px_30px_rgba(0,0,0,0.8)] selection:bg-white/30 !text-transparent bg-clip-text bg-gradient-to-b from-white via-white to-white/70"
              ...
              <div className="flex flex-wrap items-center gap-3 sm:gap-4 text-xs sm:text-sm font-semibold text-white/90 mb-6 mt-4">
                 <span className="liquid-glass px-3 py-1.5 rounded-md flex items-center gap-1"><span className="text-yellow-400">★</span> {currentMovie.rating}</span>
                 <span className="liquid-glass px-3 py-1.5 rounded-md">{currentMovie.year}</span>
                 {currentMovie.quality && <span className="px-3 py-1.5 liquid-glass rounded-md uppercase tracking-wider">{currentMovie.quality}</span>}
                 {currentMovie.seasons && <span className="liquid-glass px-3 py-1.5 rounded-md">{currentMovie.seasons.length} Seasons</span>}
                 <span className="text-white/80 ml-2 text-sm drop-shadow-md">{currentMovie.genre?.join(' • ')}</span>
              </div>

              <p className="text-sm sm:text-lg font-medium text-white/80 max-w-2xl leading-relaxed drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] line-clamp-3 md:line-clamp-4 mb-10">
              ...
              <div className="flex flex-wrap items-center gap-4">
                <button 
                  ...
                  className="bg-white text-black px-8 py-4 sm:px-10 rounded-full font-bold flex items-center gap-3 hover:bg-gray-200 transition-colors duration-300 hover:scale-105 active:scale-95 shadow-[0_0_40px_rgba(255,255,255,0.4)] text-lg"
                >
                  <Play className="w-6 h-6 fill-current" /> Play Now
                </button>
                <button 
                  onClick={() => onMovieClick?.(currentMovie)}
                  className="liquid-glass text-white px-8 py-4 sm:px-10 rounded-full font-semibold flex items-center gap-3 hover:bg-white/10 transition-colors duration-300 hover:scale-105 active:scale-95 text-lg"
                >
        ...
        <div className="hidden lg:block w-72 xl:w-[450px] shrink-0 z-30 perspective-1000 mb-8 pt-10">
          <AnimatePresence mode="wait">
            <motion.div 
      ...
      <button 
        onClick={handlePrev}
        className="absolute left-6 top-1/2 -translate-y-1/2 z-30 p-3 sm:p-4 rounded-full liquid-glass text-white/80 hover:text-white opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-all duration-300 hidden sm:block"
      >
        <ChevronLeft className="w-8 h-8" />
      </button>
      <button 
        onClick={handleNext}
        className="absolute right-6 top-1/2 -translate-y-1/2 z-30 p-3 sm:p-4 rounded-full liquid-glass text-white/80 hover:text-white opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-all duration-300 hidden sm:block"
      >
        <ChevronRight className="w-8 h-8" />
      </button>

      {/* Interactive Progress Slider */}
      <div className="absolute bottom-6 left-0 right-0 z-40 w-full px-6 sm:px-10 lg:px-16 mx-auto max-w-[1600px] flex gap-3 h-1.5 pointer-events-none">
         {heroMovies.map((_, index) => (
           <div 
             key={index}
             onClick={() => setCurrentIndex(index)}
             className="flex-1 h-full bg-white/20 overflow-hidden cursor-pointer hover:bg-white/40 transition-colors duration-300 relative pointer-events-auto rounded-full shadow-[0_2px_10px_rgba(0,0,0,0.5)]"
           >
             {index === currentIndex && (
               <motion.div 
